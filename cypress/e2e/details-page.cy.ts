...
describe('Book Details Page', () => {
    ...
    it('should display the correct details for the clicked book', () => {
      ...
      cy.get('[data-cy="book-author"]').should('contain.text', expectedBook.author.name);
      cy.get('[data-cy="book-genre"]').should('contain.text', expectedBook.genre);
      cy.get('[data-cy="book-publication-date"]').should('contain.text', expectedBook.publication_date);
      cy.get('[data-cy="book-description"]').should('contain.text', expectedBook.description);
    });
  });
